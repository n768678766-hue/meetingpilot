...
from meetingpilot.storage import load_bad_cases, load_meeting_history, save_bad_case, save_meeting_result
...
DATA_DIR = Path("data")
SAMPLE_PATH = Path("samples/sample-meeting.txt")
...
if result is not None:
    ...
    with ec1:
        st.download_button(
            ...
            file_name=f"{result.title.lower().replace(' ', '-')}.md",
            mime="text/markdown",
            use_container_width=True,
        )
    with ec2:
        st.download_button(
            ...
            file_name=f"{result.title.lower().replace(' ', '-')}-actions.csv",
            mime="text/csv",
            use_container_width=True,
        )
